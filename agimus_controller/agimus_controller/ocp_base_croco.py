...
class OCPCrocoBase(OCPBase):
    def __init__(self, rmodel: pin.Model, cmodel: pin.GeometryModel, OCPParams: OCPParamsCrocoBase) -> None:
    ...
    def x_init(self) -> np.ndarray:
        ...
        for TrajectoryPoint in self._OCPParams.WeightedTrajectoryPoints:
            x = np.array(TrajectoryPoint.point.robot_configuration + TrajectoryPoint.point.robot_velocity)
            x_init.append(x)
        return x_init
    ...
    def u_init(self) -> np.ndarray:
        ...
        u_init = [np.zeros(self._rmodel.nq) for _ in range(self.horizon_size) - 1] # For the panda nu = nq, but i'm not sure nu exists in pinocchio
        return u_init
    ...
    def solve(
        ...
        for t in range(self.horizon_size):
            ...
            if t < self.horizon_size - 1:
                ...
                runningCostModel.addCost("stateReg", xRegCost, np.concatenate([self._OCPParams.WeightedTrajectoryPoints[t].weight.w_robot_configuration, self._OCPParams.WeightedTrajectoryPoints[t].weight.w_robot_velocity]))
                runningCostModel.addCost("ctrlRegGrav", uRegCost, self._OCPParams.WeightedTrajectoryPoints[t].weight.w_robot_effort)
                runningCostModel.addCost(
                    "gripperPoseRM", goalTrackingCost, self._OCPParams.WeightedTrajectoryPoints[t].weight.w_end_effector_poses
                )
                ...
                terminalCostModel.addCost("stateReg", xRegCost, np.concatenate([self._OCPParams.WeightedTrajectoryPoints[t].weight.w_robot_configuration, self._OCPParams.WeightedTrajectoryPoints[t].weight.w_robot_velocity]))
                terminalCostModel.addCost(
                    "gripperPose", goalTrackingCost, self._OCPParams.WeightedTrajectoryPoints[t].weight.w_end_effector_poses
                )
