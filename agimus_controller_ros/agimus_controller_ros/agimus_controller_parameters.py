    ...
    class ParamListener:
        ...
        def declare_params(self):
            ...
            if not self.node_.has_parameter(self.prefix_ + "free_flyer"):
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "free_flyer", parameter, descriptor
                )
            ...
            if not self.node_.has_parameter(self.prefix_ + "collision_as_capsule"):
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "collision_as_capsule", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "self_collision"):
                descriptor = ParameterDescriptor(
                    description="Wether we account for self-collision", read_only=False
                )
                parameter = updated_params.self_collision
                self.node_.declare_parameter(
                    self.prefix_ + "self_collision", parameter, descriptor
                )
